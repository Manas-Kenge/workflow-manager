...
DEPLOY_ENV=prod
DEPLOY_HOST=workflow-manager.example.com
DEPLOY_PORT=22
DEPLOY_USER=plone
...
You need either a Ubuntu or Debian based system for workflow-manager.example.com, enable SSH, and install a supported version of Python 3 on that system.
...
prod:
  hosts:
    workflow-manager.example.com:
      ansible_user: root
      host: workflow-manager
      hostname: workflow-manager.example.com
```
...
Deploy the stack defined in `devops/stacks/workflow-manager.example.com.yml` to the remote server with:
