...
STACK_NAME=workflow-manager-example-com
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://workflow-manager.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t collective/workflow-manager-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t collective/workflow-manager-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name workflow-manager-frontend-acceptance --link workflow-manager-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d collective/workflow-manager-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name workflow-manager-backend-acceptance -d collective/workflow-manager-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop workflow-manager-frontend-acceptance
	@docker stop workflow-manager-backend-acceptance
